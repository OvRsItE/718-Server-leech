...
//	}
	...
	public void sendPublicMessage(Player p, PublicChatMessage message) {
		...
		if (message instanceof QuickChatMessage) {
			...
			int offset = 1 + Huffman.encryptMessage(1, message.getMessage(player.isFilteringProfanity()).length(),
					chatStr, 0, message.getMessage(player.isFilteringProfanity()).getBytes());
